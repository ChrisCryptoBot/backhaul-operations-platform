...
import { listBrokers } from "@/server/reference";
...
export default async function BrokersPage() {
  ...
  try {
    ...
    return (
      <main className="db-root db-fallback-main">
        <AuthErrorState title="Brokers" description={description} />
      </main>
    );
  }
  ...
  return (
    <AppShell title="Brokers" viewerIsAdmin={viewerIsAdmin} viewerCanManageReference={canWrite} regionCode={getPhase1RegionCode()}>
      <BrokersManager initialBrokers={brokers} canWrite={canWrite} />
    </AppShell>
  );
}
