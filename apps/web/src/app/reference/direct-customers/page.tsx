...
import { listDirectCustomers } from "@/server/reference";
import { DirectCustomersManager } from "./direct-customers-manager";

export default async function DirectCustomersPage() {
  ...
  try {
    ...
    return (
      <main className="db-root db-fallback-main">
        <AuthErrorState title="Direct customers" description={description} />
      </main>
    );
  }
  ...
  return (
    <AppShell title="Direct customers" viewerIsAdmin={viewerIsAdmin} viewerCanManageReference={canWrite} regionCode={getPhase1RegionCode()}>
      <DirectCustomersManager initialDirectCustomers={directCustomers} canWrite={canWrite} />
    </AppShell>
  );
}
