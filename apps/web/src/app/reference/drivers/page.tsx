...
import { listDrivers, listDropLots } from "@/server/reference";
  ...
  try {
    ...
    return (
      <main className="db-root db-fallback-main">
        <AuthErrorState title="Drivers" description={description} />
      </main>
    );
  }

  const [drivers, dropLots] = await Promise.all([listDrivers({ regionId }), listDropLots({ regionId })]);

  return (
    <AppShell title="Drivers" viewerIsAdmin={viewerIsAdmin} viewerCanManageReference={canWrite} regionCode={getPhase1RegionCode()}>
      <DriversManager initialDrivers={drivers} dropLots={dropLots} canWrite={canWrite} />
    </AppShell>
  );
}
