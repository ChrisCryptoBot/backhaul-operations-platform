...
import { directCustomerMutationSchema } from "@/contracts/reference";
import {
  createDirectCustomer,
  listDirectCustomers,
  softDeleteDirectCustomer,
...
export async function GET(request: Request) {
  try {
    ...
    const directCustomers = await listDirectCustomers({ regionId });
    return NextResponse.json({ regionId, directCustomers }, { status: 200 });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const actorUserId = userId ?? "dev-bypass-user";
    const body = directCustomerMutationSchema.parse(await request.json());
    ...
    if (body.action === "create_direct_customer") {
      await createDirectCustomer({
        regionId,
        actorId: actorUserId,
        fields: {
          name: body.directCustomer.name,
          cadenceCount: body.directCustomer.cadenceCount ?? null,
          cadencePeriod: body.directCustomer.cadencePeriod ?? null,
          notes: body.directCustomer.notes ?? null
        }
      });
    } else if (body.action === "update_direct_customer") {
      await updateDirectCustomer({
        regionId,
        actorId: actorUserId,
        directCustomerId: body.directCustomerId,
        fields: body.fields
      });
    } else if (body.action === "delete_direct_customer") {
      await softDeleteDirectCustomer({
        regionId,
        actorId: actorUserId,
        directCustomerId: body.directCustomerId,
        reason: body.reason
