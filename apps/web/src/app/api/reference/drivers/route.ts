...
import { driverMutationSchema } from "@/contracts/reference";
import { createDriver, listDrivers, softDeleteDriver, updateDriver } from "@/server/reference";
...
export async function GET(request: Request) {
  try {
    ...
    const drivers = await listDrivers({ regionId });
    return NextResponse.json({ regionId, drivers }, { status: 200 });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    const actorUserId = userId ?? "dev-bypass-user";
    const body = driverMutationSchema.parse(await request.json());
    ...
    if (body.action === "create_driver") {
      await createDriver({
        regionId,
        actorId: actorUserId,
        fields: {
          code: body.driver.code,
          fullName: body.driver.fullName,
          phone: body.driver.phone ?? null,
          homeDropLotId: body.driver.homeDropLotId ?? null,
          active: body.driver.active ?? true,
          attributes: body.driver.attributes ?? [],
          scheduleDays: body.driver.scheduleDays ?? [],
          scheduleStart: body.driver.scheduleStart ?? null,
          scheduleTimeZone: body.driver.scheduleTimeZone ?? null,
          scheduleNote: body.driver.scheduleNote ?? null
        }
      });
    } else if (body.action === "update_driver") {
      await updateDriver({ regionId, actorId: actorUserId, driverId: body.driverId, fields: body.fields });
    } else if (body.action === "delete_driver") {
      await softDeleteDriver({ regionId, actorId: actorUserId, driverId: body.driverId, reason: body.reason });
    }
    ...
    if (error instanceof Error && (error.message.includes("in use") || error.message.includes("already exists"))) {
